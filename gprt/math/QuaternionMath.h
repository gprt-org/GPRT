...
template<typename T>
struct std::equal_to<::math::quat<T>>
{
    constexpr bool operator()(const ::math::quat<T>& lhs, const ::math::quat<T>& rhs) const
    {
...
template<typename T>
struct std::not_equal_to<::math::quat<T>>
{
    constexpr bool operator()(const ::math::quat<T>& lhs, const ::math::quat<T>& rhs) const
    {
        return lhs.x != rhs.x || lhs.y != rhs.y || lhs.z != rhs.z || lhs.w != rhs.w;
...
template<typename T>
struct std::hash<::math::quat<T>>
{
    constexpr size_t operator()(const ::math::quat<T>& v) const
    {
        size_t result = 0;
