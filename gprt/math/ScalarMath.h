...
#include <bit.h> // TODO C++20: Replace with <bit>
#include <cmath>
...
template<> [[nodiscard]] inline float saturate(float x) noexcept { return std::max(0.f, std::min(1.f, x)); }
template<> [[nodiscard]] inline double saturate(double x) noexcept { return std::max(0.0, std::min(1.0, x)); }
